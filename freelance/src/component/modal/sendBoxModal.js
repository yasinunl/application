...
const SendBoxModal = () => {
  ...
  const {getSendFocusedMessage} = useContext(ApiContext)
  ...
  const handleSubmit = async(e)=>{
    ...
    axios.post("/send/sendagain", bodyFormData).then(()=>{
      console.log("trtewds")
      setSuccessMg(true)
      setAlertHidden(true)
    })
  ...
  //
  const getMessage = async()=> {setMessage(await getSendFocusedMessage(inboxMessageID))}
        ...
        <div className="flex justify-between items-center h-20 border-b-2 mb-8">
          <div className="flex space-x-4 items-center">
            ...
            <div className="flex flex-col">
              <h3 className="font-semibold text-lg"> <i className="font-thin">To - </i> {message ? message.name : "Name"}</h3>
              <p className="text-light text-gray-400">
                {message ? message.date : "Date"}{console.log(message)}
              </p>
            </div>
          </div>
        </div>
